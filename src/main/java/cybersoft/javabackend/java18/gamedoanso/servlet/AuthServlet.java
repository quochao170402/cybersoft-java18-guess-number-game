...
public class AuthServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        switch (request.getServletPath()) {
            case UrlUtils.SIGN_UP -> request.getRequestDispatcher(JspUtils.SIGN_UP)
                    .forward(request, response);
            case UrlUtils.SIGN_IN -> request.getRequestDispatcher(JspUtils.SIGN_IN)
                    .forward(request, response);
    ...
    private void processSignUp(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
        ...
        if (player != null) {
            ...
            request.getSession().setAttribute("errors", "Thông tin người chơi không hợp lệ hoặc đã được sử dụng");
